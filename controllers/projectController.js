...
exports.generateProject = async function (req, res) {
  ...
  await project
    .findOne({ project_title: title }, async function (err, foundProject) {
      if (err) {
        console.log(err);
      } else {
        if (foundProject) {
          ...
          await teacher
            .find({}, async function (err, result) {
              if (err) {
                ...
                await result.map(function (each) {
                  each.topics.map((ech) =>
                    teacherDomainNames.push(ech.topic_title)
                  );
                });
                spare = [...new Set(teacherDomainNames)];
                res.render("create_project", {
          ...
          try {
            ...
            const filteredTeachers = await teacher.find({
              topics: {
                $elemMatch: {
                  topic_title: major,
                  remainingProjects: { $gt: 0 },
                },
              },
            });
            const filteredTeacherNames = filteredTeachers.map(
...
exports.requestTeacher = async function (req, res) {
  ...
  await teacher
    .findOne({ username: selectedTeacher }, async function (err, foundTeacher) {
      if (err) {
        console.log(err);
      } else {
  ...
  await teacher.updateOne(
    {
      ...
      "topics.topic_title": majorOfTeacherToBeDecreased,
    },
    { $inc: { "topics.$.remainingProjects": -1 } }
  );
...
exports.showProject = async function (req, res) {
  ...
  await project
    .findOne({ _id: id }, function (err, foundProject) {
      if (err) {
        console.log(err);
      } else {
        res.render("single_project_screen", {
          title: foundProject.project_title,
          major: foundProject.project_major,
          content: foundProject.project_description,
          team: foundProject.project_team,
          User: req.session.username,
        });
      }
    })
...
exports.changeProject = async function (req, res) {
  ...
  await project
    .findOne({ project_title: name }, function (err, foundProject) {
      if (err) {
        console.log(err);
      } else {
        res.render("editable_project", {
          condition: false,
          message: "",
...
exports.editProject = async function (req, res) {
  ...
  await project
    .findOne({ _id: id }, async function (err, foundProject) {
      if (err) {
        console.log(err);
      } else {
        await team
          .findOne(
            { team_name: foundProject.project_team },
            async function (err, foundTeam) {
              if (err) {
                console.log(err);
              } else {
                if (foundTeam.team_leader === req.session.username) {
                  ...
                  res.render("editable_project", {
                    condition: true,
                    message: "Only team leader can edit project",
                    project_id: foundProject._id,
                    major: foundProject.project_major,
                    project_title: foundProject.project_title,
                    project_description: foundProject.project_description,
